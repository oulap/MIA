...
$w_0, \ldots, w_D$
are tunable weights that need to be estimatated from the 
% $N$ 
...
% \footnote{
In particular, they often allow to compress signals efficiently by discarding the highest frequencies; $\bldgr{\Phi} \fat{w}$ and $\bldgr{\Phi}\transp \fat{t}$ can be computed very quickly using the fast Fourier transform (FTT); and $\bldgr{\Phi}\transp\bldgr{\Phi} = N \fat{I}$.
% }. 
%
...
The amount of smoothing that is applied can be controlled by both the number of basis functions $M$ and the regularization parameter $\lambda$: chosing higher $M$ or lower $\lambda$ will produce less smoothing. 
...
\label{eq:vec}
\end{equation}
In this re-arrangment, element $t_{{n_1},{n_2}}$ in $\fat{T}$ corresponds to element $t_n$ in $\fat{t}$, where $n = n_1 + (n_2-1) N_1$.

%
In order to proceed, we also need to choose appropriate basis functions 
$\phi_m(\fat{x})$ that work in 2D. A convenient choice is often \emph{separable} basis functions, which are simply the product of two 1D basis funcions (one taking as input $x_1$, and the other one $x_2$). If there are $M_1$ basis functions in the first (row) direction, and $M_2$ in the second (column) direction, 
...
multiple images 
% of the same anatomy 
to each other, we'll need to differentiate the concept of ``voxel coordinates'' (which are defined in integers units, and index indivual voxels in a 3D image grid) from that of 
...
In order to compute the voxel coordinate $\fat{v}_{T2}$ in the T2-weighted scan corresponding to a s
% pecific anatomical location with 
...
This technique, which uses so-called homogeneous coordinates (vectors are augmented with a $1$ at the end), absorbes the addtion of the vector $\fat{t}$ into a single matrix multiplication, which makes concatenating several affine matrices very easy:  
...
%
Referrring to~\eqref{eq:mapping},
...
% $\delta(\fat{x}, \fat{w}_d)$
$\delta$
is modeled:
\begin{equation}
y_d( \fat{x}, \fat{w}_d ) = x_d + \delta(\fat{x}, \fat{w}_d),
\quad
\text{where}
\quad
\delta(\fat{x}, \fat{w}_d) =
...
\end{equation}
Here $\phi_m(\fat{x})$ are $M$ basis functions, which are typically taken to be \emph{seperable} (cf.~\eqref{eq:basisFunction2D} and Fig.~\ref{fig:basisFunctions2D_BSpline}),
with weights $\fat{w}_d = (w_{d,0}, \ldots w_{d,M-1})\transp$.
%
The interpretation of these weights is that, when they are all set to zero (i.e., when $\fat{w}_d = \fat{0}$), the deformation $\delta$ is also zero and no deformation is applied.
...
Here $\| \fat{a} - \fat{b}\|^2 = \sum_{d=1}^D (a_1-b_1)^ 2$ measures the squared Eucledian distance between two points $\fat{a}$ and $\fat{b}$.
...
\sum_{n=1}^N 
\sum_{d=1}^D
\left[ y_{n,d} - y_d( \fat{x}_n, \fat{w_d} ) \right]^2 
\\
& = & 
...
$\fat{y}_d( \fat{x}_n, \fat{w_d} )$ is only \emph{linearly} dependent on $\fat{w}_d$,
...
\text{and}
\quad
\sum_{n=1}^N \fat{x}_n \fat{y}_n\transp 
=
\fat{U}
\bldgr{\Sigma}
\fat{V}\transp
...
\iffalse 
\begin{equation}
y_d( \fat{x}, \fat{w}_d ) = x_d + \epsilon(\fat{x}, \fat{w}_d),
\quad
\text{where}
\quad
\delta(\fat{x}, \fat{w}_d) =
...
When two images need to be aligned that were acquired with two different imaging modalities (e.g., CT vs.~MR, or MR vs.~PET), the intensity characteristics of most anatomical structures will generally be quite diferent between the two images. This precludes the use of the sum-of-squared-differences energy as a registration criterion.
...
In constrast, when the images are moved out of alignment, intensity pairs will become more variable, ``smearing out'' the counts from high-count histogram entries into (what were previously) low-count bins. This process is illustrated in Fig.~\ref{fig:registration_histograms_blurring}.
%
...
In order to avoid such pathologoical solutions, it is customary to alter the energy as follows:
\begin{equation}
...
\begin{figure}
  ...
  (\subref{fig:logisticRegressionImageClassification_c}) Final segmentation produced by the trained classifier, overlayed on top of the input image.
...
\emph{separable} basis functions, which are simply the product of 1D basis funcions as in~\eqref{eq:basisFunction2D}.
% 
